...
/*
 * Backport std::make_shared from C++14.
 *
...
make_shared(std::size_t n) {
  ...
  return std::shared_ptr<T>(new RT[n]);
}
...
#ifdef CPPHTTPLIB_ZLIB_SUPPORT
      decompressor = detail::make_shared<gzip_decompressor>();
#else
      status = 415;
      return false;
#endif
...
#ifdef CPPHTTPLIB_BROTLI_SUPPORT
      decompressor = detail::make_shared<brotli_decompressor>();
#else
      status = 415;
      return false;
#endif
...
Response::set_content_provider(const char *content_type,
  ...
  set_header("Content-Type", content_type);
  content_length_ = 0;
  content_provider_ = detail::ContentProviderAdapter(provider);
  content_provider_resource_releaser_ = resource_releaser;
  is_chunked_content_provider_ = false;
}
...
inline bool Server::set_mount_point(const char *mount_point, const char *dir,
  ...
  if (detail::is_dir(dir)) {
    ...
    if (!mnt.empty() && mnt[0] == '/') {
      base_dirs_.push_back({mnt, dir, (headers)});
      return true;
    }
  }
  return false;
...
Server::set_expect_100_continue_handler(Expect100ContinueHandler handler) {
  expect_100_continue_handler_ = (handler);
}
...
#ifdef CPPHTTPLIB_ZLIB_SUPPORT
        compressor = detail::make_shared<detail::gzip_compressor>();
#endif
      } else if (type == detail::EncodingType::Brotli) {
#ifdef CPPHTTPLIB_BROTLI_SUPPORT
        compressor = detail::make_shared<detail::brotli_compressor>();
#endif
      } else {
        compressor = detail::make_shared<detail::nocompressor>();
      }
...
inline std::shared_ptr<Response> ClientImpl::send_with_content_provider(
...
#endif
  {
    if (content_provider) {
      ...
      req.content_provider_ = (content_provider);
  ...
  auto res = detail::make_shared<Response>();
...
inline Result ClientImpl::Get(const char *path,
                              ResponseHandler response_handler,
                              ContentReceiver content_receiver) {
  return Get(path, Headers(), (response_handler),
...
inline Result ClientImpl::Get(const char *path, const Headers &headers,
  ...
  req.headers = default_headers_;
  req.headers.insert(headers.begin(), headers.end());
  req.response_handler_ = (response_handler);
  req.content_receiver_ =
...
inline Result ClientImpl::Post(const char *path,
                               ContentProviderWithoutLength content_provider,
                               const char *content_type) {
  return Post(path, Headers(), (content_provider), content_type);
}
...
inline Result ClientImpl::Post(const char *path, const Headers &headers,
  ...
  auto ret = send_with_content_provider(
      ...
      (content_provider), nullptr, content_type);
  return Result{(ret), get_last_error()};
}
...
inline Result ClientImpl::Put(const char *path, size_t content_length,
                              ContentProvider content_provider,
                              const char *content_type) {
  return Put(path, Headers(), content_length, (content_provider),
             content_type);
}
...
inline Result ClientImpl::Put(const char *path,
                              ContentProviderWithoutLength content_provider,
                              const char *content_type) {
  return Put(path, Headers(), (content_provider), content_type);
}
...
inline Result ClientImpl::Patch(const char *path,
                                ContentProviderWithoutLength content_provider,
                                const char *content_type) {
  return Patch(path, Headers(), (content_provider), content_type);
}
...
inline Client::Client(const char *scheme_host_port,
  ...
  if (std::regex_match(scheme_host_port, m, re)) {
    ...
    if (is_ssl) {
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
      cli_ = detail::make_shared<SSLClient>(host.c_str(), port,
                                            client_cert_path, client_key_path);
      is_ssl_ = is_ssl;
#endif
    } else {
      cli_ = detail::make_shared<ClientImpl>(host.c_str(), port,
                                             client_cert_path, client_key_path);
    }
  } else {
    cli_ = detail::make_shared<ClientImpl>(scheme_host_port, 80,
                                           client_cert_path, client_key_path);
  }
}

inline Client::Client(const std::string &host, int port)
    : cli_(detail::make_shared<ClientImpl>(host, port)) {}

inline Client::Client(const std::string &host, int port,
                      const std::string &client_cert_path,
                      const std::string &client_key_path)
    : cli_(detail::make_shared<ClientImpl>(host, port, client_cert_path,
                                           client_key_path)) {}
...
inline Result Client::Get(const char *path, ResponseHandler response_handler,
                          ContentReceiver content_receiver) {
  return cli_->Get(path, (response_handler),
                   (content_receiver));
}
inline Result Client::Get(const char *path, const Headers &headers,
                          ResponseHandler response_handler,
                          ContentReceiver content_receiver) {
  return cli_->Get(path, headers, (response_handler),
                   (content_receiver));
}
inline Result Client::Get(const char *path, ResponseHandler response_handler,
                          ContentReceiver content_receiver, Progress progress) {
...
inline Result Client::Post(const char *path, size_t content_length,
                           ContentProvider content_provider,
                           const char *content_type) {
  return cli_->Post(path, content_length, (content_provider),
                    content_type);
}
inline Result Client::Post(const char *path,
                           ContentProviderWithoutLength content_provider,
                           const char *content_type) {
  return cli_->Post(path, (content_provider), content_type);
}
inline Result Client::Post(const char *path, const Headers &headers,
                           size_t content_length,
                           ContentProvider content_provider,
...
inline Result Client::Post(const char *path, const Headers &headers,
                           ContentProviderWithoutLength content_provider,
                           const char *content_type) {
  return cli_->Post(path, headers, (content_provider), content_type);
}
...
inline Result Client::Put(const char *path, size_t content_length,
                          ContentProvider content_provider,
                          const char *content_type) {
  return cli_->Put(path, content_length, (content_provider),
                   content_type);
}
inline Result Client::Put(const char *path,
                          ContentProviderWithoutLength content_provider,
                          const char *content_type) {
  return cli_->Put(path, (content_provider), content_type);
}
inline Result Client::Put(const char *path, const Headers &headers,
                          size_t content_length,
                          ContentProvider content_provider,
...
inline Result Client::Put(const char *path, const Headers &headers,
                          ContentProviderWithoutLength content_provider,
                          const char *content_type) {
  return cli_->Put(path, headers, (content_provider), content_type);
}
...
inline Result Client::Patch(const char *path, size_t content_length,
                            ContentProvider content_provider,
                            const char *content_type) {
  return cli_->Patch(path, content_length, (content_provider),
                     content_type);
}
inline Result Client::Patch(const char *path,
                            ContentProviderWithoutLength content_provider,
                            const char *content_type) {
  return cli_->Patch(path, (content_provider), content_type);
}
inline Result Client::Patch(const char *path, const Headers &headers,
                            size_t content_length,
                            ContentProvider content_provider,
...
inline Result Client::Patch(const char *path, const Headers &headers,
                            ContentProviderWithoutLength content_provider,
                            const char *content_type) {
  return cli_->Patch(path, headers, (content_provider), content_type);
}
